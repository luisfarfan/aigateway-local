...
class CrewAIOrchestratorProvider(BaseProvider):
    ...
    async def execute(self, context: ExecutionContext) -> ProviderResult:
        try:
            ...
            # Setup Tools
            async def silent_progress(p, m):
                pass

            async def silent_artifact(k, t, m):
                pass
            ...
            if audio_key:
                ...
                return ProviderResult(
                    success=False, error_message=f"Failed to extract audio key from: {audio_res}"
                )
