...
class ImageGenerationTool(AIGatewayTool):
    ...
    description: str = (
        "Generates an image from a prompt. Returns the storage_key of the generated image."
    )
...
class VideoAssemblyTool(AIGatewayTool):
    ...
    class ToolInput(BaseModel):
        scenes: list[dict[str, Any]] = Field(
            ..., description="List of dicts with image_key, audio_key, and text."
        )
